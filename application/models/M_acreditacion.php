...
class M_acreditacion extends CI_Model { 
   ...
   public function get_estudiantes_plantel_semestre($plantel,$semestre,$id_componente){
    ...
    if(intval($semestre)<6){
        return $this->db->query("select * from Estudiante where semestre_en_curso=".$semestre." and Plantel_cct_plantel='".$plantel."' and (tipo_ingreso='NUEVO INGRESO' or tipo_ingreso='REINGRESO' or tipo_ingreso='INCORPORADO' or tipo_ingreso='TRASLADO' or tipo_ingreso='PORTABILIDAD' or tipo_ingreso='REPETIDOR') and no_control not in (select distinct Estudiante_no_control from Grupo_Estudiante as ge inner join Grupo as g on ge.Grupo_id_grupo=g.id_grupo where semestre=".$semestre." and g.estatus=1) order by primer_apellido,segundo_apellido,nombre")->result(); 
    }
